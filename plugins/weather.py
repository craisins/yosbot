...
@hook.api_key('wunderground')
@hook.command(autohelp=False)
def forecast(inp, chan='', nick='', reply=None, db=None, api_key=None):
    ".weather <location> [dontsave] | @<nick> -- gets weather data from Wunderground "\
            "http://wunderground.com/weather/api"
    ...
    for forecast in parsed_json['forecast']['simpleforecast']['forecastday']:
        fcst = '\x02' + forecast['date']['weekday_short'] + '\x02 Low: ' + str(forecast['low']['fahrenheit']) + 'F/' + str(forecast['low']['celsius'])
        fcst +='C, High: '+ str(forecast['high']['fahrenheit']) + 'F/' + str(forecast['high']['celsius']) + 'C, Humidity: ' + str(forecast['avehumidity']) + '%'
        fcst +=', Condition: ' + forecast['conditions']
        forecasts.append(fcst)
    reply(' | '.join(forecasts))
