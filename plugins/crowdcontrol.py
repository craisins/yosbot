...
#   mute_length: Integer. (optional) Length of time (seconds) to mute user.


@hook.regex(r'.*')
def crowdcontrol(inp, kick=None, ban=None, unban=None, mute=None, unmute=None, reply=None, bot=None):
    inp = inp.group(0)
    for rule in bot.config.get('crowdcontrol', []):
        if re.search(rule['re'], inp) is not None:
            ...
            mute_length = rule.get('mute_length', 0)
            reason = rule.get('msg')
            if ban_length != 0:
                ban()
            if should_kick:
